...
// Suite will not pass because of state leakage
describe('The PersonManager() class', () => {
  const personManager = setupPersonManager([{ name: 'Bob', age: 38 }]);
  ...
  it.skip('ageEveryoneBy() method ages all persons by the specified amount', () => {
    personManager.ageEveryoneBy(10);
    expect(personManager.persons[0].age).toEqual(48);
    // FAIL: Expected vale to equal: 48, Received: 49;
  });
});
